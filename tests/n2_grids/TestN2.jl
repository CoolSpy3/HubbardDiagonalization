...
function (@main)(args)
	...
	for test_set in grids_zip.files
		...
		for (i, T) in enumerate(T_vals)
			...
			results = HubbardDiagonalization.diagonalize_and_compute_observables(u_vals, test_config, graph, observables...)
			...
			for (observable_name, results_file) in test_observables
				...
				push!(generated_statistics[observable_name], [mean_difference, std_difference, max_difference])
			end
		end
	end
